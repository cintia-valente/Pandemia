...
export async function authUser(req: express.Request, res: express.Response, next: express.NextFunction) {
   ...
   if (!user) 
      return res.status(400).send({ error: 'User not found' }); //se não encontrou o usuário, retorna uma mensagem de erro
...
export async function authMiddleware(req: express.Request, res: express.Response, next: express.NextFunction) {
   ...
   jwt.verify(token, authConfig.secret, (err, decoded) => {
      if (err) 
         return res.status(401).send({ error: 'Token invalid' }); //se o token tiver algum erro, retorna uma mensagem de erro
      req.params.userId = (<any>decoded)._id; //se estiver certo, então inclui o id nas próx requisições
      return next();
   });
}
